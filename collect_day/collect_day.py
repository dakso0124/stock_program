...
class DayCollect:
    ...
    def reqeustData(self, obj, code):
        ...
        print('가져온 데이터량 :', self.count, "통신상태", self.rqStatus, obj.GetDibMsg1())
    ...
    def start_get_days_data(self, codes):
        ...
        for index, code in enumerate(codes):          # 반복문을 통해
            ...
            if self.reqeustData(self.objStockWeek, code):     # Data를 가져오는데 성공하면
                # 연속 데이터 요청
                while self.objStockWeek.Continue:       # 연속 조회처리
                    ret = self.reqeustData(self.objStockWeek, code)
        self.conn.close()